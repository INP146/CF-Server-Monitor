...
export const BILLING_CYCLES: readonly BillingCycleOption[] = Object.freeze([
...
export const CURRENCY_OPTIONS: readonly CurrencyOption[] = Object.freeze([
...
export function normalizePrice(value: unknown): string {
  ...
  const numberText = String(value).trim().match(/-?[\d.,]+/)?.[0]
...
export function normalizeCurrency(value: unknown): string {
  ...
  const multiCharacter = CURRENCY_OPTIONS.find((item) => item.symbol.length > 1 && normalized.startsWith(item.symbol))
  if (multiCharacter) return multiCharacter.symbol
  return NORMALIZED_CURRENCIES.has(normalized[0]!) ? normalized[0]! : ''
}
...
export function detectCurrencySymbol(value: unknown): string {
  ...
  const multiCharacter = CURRENCY_OPTIONS.find((item) => item.symbol.length > 1 && raw.includes(item.symbol))
  return multiCharacter?.symbol
    ?? CURRENCY_OPTIONS.find((item) => item.symbol.length === 1 && raw.includes(item.symbol))?.symbol
    ?? ''
}
...
export function normalizeBillingCycle(value: unknown): string {
  ...
  if (!raw) return 'month'
  return BILLING_CYCLES.find((item) => item.value === raw)?.value
    ?? CYCLE_ALIASES.get(raw.toLowerCase())
    ?? 'month'
}

export function getBillingCycleOption(value: unknown): BillingCycleOption {
...
function addBillingCycleToDate(dateString: string, billingCycle: unknown): string {
  const parsed = parseDateOnly(dateString)
  if (!parsed) return dateString.trim()
  const zeroBasedMonth = parsed.month - 1 + getBillingCycleOption(billingCycle).months
  const year = parsed.year + Math.floor(zeroBasedMonth / 12)
  const month = ((zeroBasedMonth % 12) + 12) % 12 + 1
  const daysInMonth = new Date(Date.UTC(year, month, 0)).getUTCDate()
  return toDateString(year, month, Math.min(parsed.day, daysInMonth))
}
...
export function renewExpireDateIfNeeded(
  ...
  const renewalDate = new Date(now + (Number(renewBeforeDays) || 0) * 86_400_000)
  const renewalDateString = toDateString(
    renewalDate.getUTCFullYear(),
    renewalDate.getUTCMonth() + 1,
    renewalDate.getUTCDate(),
  )
  let nextDate = original
  let guard = 0
  while (nextDate <= renewalDateString && guard < 1_200) {
    nextDate = addBillingCycleToDate(nextDate, billingCycle)
    guard += 1
  }
  return { expire_date: nextDate, renewed: nextDate !== original }
}

export function formatBillingPrice(
  server: Pick<DashboardServer, 'price' | 'currency' | 'billing_cycle'>,
  language: 'zh' | 'en' = 'zh',
): string {
  const price = normalizePrice(server.price as NumericValue)
  if (!price) return ''
  if (isFreePrice(price)) return language === 'zh' ? '免费' : 'Free'
  ...
  return `${currency}${price}/${language === 'zh' ? cycle.shortLabelZh : cycle.shortLabelEn}`
}
